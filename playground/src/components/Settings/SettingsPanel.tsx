...
export function SettingsPanel({ settings, onUpdate, onClose }: Props) {
  const activeSkills = settings.activeSkills ?? ['blazer-engine']
  const customSkills: CustomSkill[] = settings.customSkills ?? []
  const [newSkill, setNewSkill] = useState<{ name: string; description: string; prompt: string }>({
    name: '', description: '', prompt: '',
  })
  const [addingSkill, setAddingSkill] = useState(false)
  const [editingSkill, setEditingSkill] = useState<Skill | null>(null)
  const [editDraft, setEditDraft] = useState<{ name: string; description: string; prompt: string }>({
    name: '', description: '', prompt: '',
  })
  ...
  const saveCustomSkill = () => {
    ...
    onUpdate({
      customSkills: [...customSkills, skill],
      activeSkills: [...activeSkills, skill.id],
    })
  ...
  const saveEdit = () => {
    ...
    if (editingSkill.builtIn) { setEditingSkill(null); return }
    onUpdate({
      customSkills: customSkills.map((s) =>
        s.id === editingSkill.id
          ? { ...s, name: editDraft.name.trim(), description: editDraft.description.trim(), prompt: editDraft.prompt.trim() }
          : s
      ),
    })
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div className="flex items-center gap-2">
              ...
              {editingSkill.builtIn && (
                <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded font-medium">built-in · read only</span>
              )}
            </div>
          ...
          <div className="p-5 space-y-3 overflow-y-auto flex-1">
            <div>
              ...
              <input
                ...
                className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-900/10 disabled:bg-gray-50 read-only:bg-gray-50 read-only:text-gray-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                readOnly={editingSkill.builtIn}
                className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-900/10 read-only:bg-gray-50 read-only:text-gray-500"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setEditDraft((p) => ({ ...p, prompt: e.target.value }))}
                readOnly={editingSkill.builtIn}
                rows={12}
                className="w-full text-xs font-mono border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-900/10 resize-none read-only:bg-gray-50 read-only:text-gray-500"
              />
            </div>
          </div>
          {!editingSkill.builtIn && (
      ...
      <div className="relative w-96 bg-white border-l border-gray-200 overflow-y-auto shadow-2xl">
        <div className="p-6">
          ...
          <ProviderToggle
            ...
            onChange={(provider) => onUpdate({ activeProvider: provider })}
          />
          ...
          <div className="mt-6">
            ...
            <div className="space-y-3">
              <ApiKeyInput
                ...
                onChange={(apiKey) => onUpdate({ openai: { ...settings.openai, apiKey } })}
                placeholder="sk-..."
              />
              <ModelSelector
                value={settings.openai.model}
                models={OPENAI_MODELS}
                onChange={(model) => onUpdate({ openai: { ...settings.openai, model } })}
              />
              <TemperatureSlider
          ...
          <div>
            ...
            <div className="space-y-3">
              <ApiKeyInput
                ...
                onChange={(apiKey) => onUpdate({ claude: { ...settings.claude, apiKey } })}
                placeholder="sk-ant-..."
              />
              <ModelSelector
                value={settings.claude.model}
                models={CLAUDE_MODELS}
                onChange={(model) => onUpdate({ claude: { ...settings.claude, model } })}
              />
              <TemperatureSlider
                value={settings.claude.temperature}
                onChange={(temperature) => onUpdate({ claude: { ...settings.claude, temperature } })}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Preferred Language</label>
              <select
                value={settings.execution.preferredLanguage}
                onChange={(e) =>
                  onUpdate({
                    execution: {
                      ...settings.execution,
                      preferredLanguage: e.target.value as 'javascript' | 'python',
                    },
                  })
                }
                className="w-full bg-white text-gray-900 rounded-lg px-3 py-2 text-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-300 transition"
              >
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="text-xs text-gray-500 hover:text-gray-900 px-2 py-0.5 rounded hover:bg-gray-100 transition"
              >
                + Add skill
              </button>
            </div>
            <p className="text-xs text-gray-400 mb-3">
              Skills inject extra knowledge into the AI's system prompt.
            </p>
                ...
                <div key={skill.id} className="flex items-start gap-2.5 p-2.5 rounded-lg border border-gray-100 bg-gray-50 group">
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-medium text-gray-800 group-hover:text-gray-900">{skill.name}</span>
                      {!skill.builtIn && (
            ...
            {addingSkill && (
              <div className="mt-3 p-3 rounded-lg border border-gray-200 bg-white space-y-2">
                <input
                  ...
                  className="w-full text-xs border border-gray-200 rounded-md px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-gray-400"
                />
                <input
                  type="text"
                ...
                />
                <textarea
                  placeholder="Skill prompt — instructions injected into the AI's system context"
                  value={newSkill.prompt}
                  onChange={(e) => setNewSkill((p) => ({ ...p, prompt: e.target.value }))}
                  rows={5}
                  className="w-full text-xs border border-gray-200 rounded-md px-2.5 py-1.5 font-mono focus:outline-none focus:ring-1 focus:ring-gray-400 resize-none"
                />
                <div className="flex gap-2">
                  <button
                    onClick={saveCustomSkill}
                    disabled={!newSkill.name.trim() || !newSkill.prompt.trim()}
                    className="flex-1 bg-gray-900 text-white text-xs rounded-md py-1.5 font-medium hover:bg-gray-700 disabled:opacity-30 transition"
                  >
                    Save skill
                  </button>
                  <button
                    onClick={() => { setAddingSkill(false); setNewSkill({ name: '', description: '', prompt: '' }) }}
                    className="text-xs text-gray-400 hover:text-gray-600 px-3 py-1.5 rounded-md hover:bg-gray-100 transition"
                  >
                    Cancel
                  </button>
                </div>
