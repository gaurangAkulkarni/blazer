...
export function DataFrameTable({ data, columns, shape }: Props) {
  ...
  return (
    <div className="my-2">
      <div className="text-xs text-gray-500 mb-1.5 flex items-center gap-2">
        <span className="font-semibold text-gray-700">DataFrame</span>
        <span>{totalRows.toLocaleString()} rows x {totalCols} cols</span>
        ...
        <table className="min-w-full text-xs">
          <thead>
            <tr className="bg-gray-50">
              ...
              {columns.map((col) => (
                <th
                  key={col}
                  className="px-3 py-2 text-left text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap"
                >
                  {col}
                </th>
              ))}
          ...
          <tbody>
            {displayRows.map((row, i) => (
              <tr
                key={i}
                className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/50 transition-colors`}
              >
                  ...
                  return (
                    <td
                      ...
                      className={`px-3 py-1.5 border-b border-gray-100 whitespace-nowrap ${
                        isNumber ? 'text-right tabular-nums text-gray-800' : 'text-gray-700'
